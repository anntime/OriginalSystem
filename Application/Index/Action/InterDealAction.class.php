...
Class InterDealAction extends Action{
	...
	Public function SaveTempInfo($data){
		$db = new \Think\Model();
		$db->execute('delete from os_temp where 1');
		// die;
		$db->execute('alter table os_temp AUTO_INCREMENT=1;');
	...
	Public function getInterReturn($RequestInfo){
		...
		//调用评估类
		import('Class.Evaluate',APP_PATH."Common/");
		//import('MyClass',APP_PATH.'Common','.php');
		$ttt = D("Evalue");
		$IndirectUserInfo = $ttt->IndirectEvaluate($IndirectUserInfo);
	...
	Public function SaveInterAction($RequestInfo,$SaveData){
		...
		$data["totalScore"] =  D("Evalue")->getTotalScore($userId);
		import("Class.RoleMap",APP_PATH);
		$data["isPass"] = D("RoleMap")->isPass($RequestInfo,$SaveData,$data["totalScore"]);
	...
	*/
	...
	Public function getStartTimeline(){
		$id = I("id");
		$time = I("time");
		import('Class.History',APP_PATH."Common/");
		$history = new \History();
	...
	Public function findNameById(){
		$id = I("id");
		$username = M("user")->where("id = {$id}")->field("username")->find();
		$this->ajaxReturn($username["username"]);
	}
}
?>